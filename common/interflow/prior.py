...
class GMM(Prior):
    def __init__(self, loc=None, var=None, scale = 1.0, ndim = None, nmix= None, device='cpu', requires_grad=False):
        ...
        self.register_buffer('loc', loc)   ### locs should have shape [n_mix, ndim]
        self.register_buffer('var', var)   ### should have shape [n_mix, ndim]
        
        if requires_grad:
            self.loc.requires_grad_()
            self.var.requires_grad_()
        
        mix = Categorical(torch.ones(self.nmix, device=self.loc.device))
